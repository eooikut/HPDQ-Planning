...
def tiendo_search():
    ...
    records = get_rows_from_db()   # giữ nguyên hàm lấy dữ liệu
    factory_map = {
        "Hòa Phát Dung Quất 2": "HRC2",
        "Hòa Phát Dung Quất": "HRC1"
    }
    for r in records:
        ...
        if f:
            f_clean = f.strip()
            r["Factory"] = factory_map.get(f_clean, f_clean)
    # filter thêm process_color
    filtered_records = [
        r for r in records
        if (not process_color or r["process_color"] == process_color)
    ]

    rows = filter_and_group(filtered_records, keyword, customer,factory)
